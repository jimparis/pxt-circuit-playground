...
// The Adafruit SAMD UF2 bootloader reads the final SRAM word during reset.
// QUICK_BOOT skips its double-tap delay when returning to the application;
// the normal double-tap sentinel makes RESET_INTO_BOOTLOADER stay in UF2/HF2.
#ifdef SAMD21
#define PXT_SAMD_BOOT_MAGIC_PTR ((volatile uint32_t *)(HMCRAMC0_ADDR + HMCRAMC0_SIZE - 4))
#else
#define PXT_SAMD_BOOT_MAGIC_PTR ((volatile uint32_t *)(HSRAM_ADDR + HSRAM_SIZE - 4))
#endif
#define PXT_SAMD_BOOTLOADER_MAGIC 0xf01669ef
#define PXT_SAMD_QUICK_BOOT_MAGIC 0xf02669ef
#define QUICK_BOOT(application)                                                                     \
    (*PXT_SAMD_BOOT_MAGIC_PTR =                                                                    \
         (application) ? PXT_SAMD_QUICK_BOOT_MAGIC : PXT_SAMD_BOOTLOADER_MAGIC)
